...
캐릭터 편집 UI는 `owner_id`가 있는 사용자 소유 항목에만 PATCH/DELETE 동작을
노출하지만, 실제 권한은 계속 백엔드 서비스가 검사합니다. 삭제 전에 화면에서 두 번
확인하고, 대화가 참조하는 캐릭터의 `409 Conflict`는 과거 대화를 먼저 정리하라는
안내로 변환합니다.

장기 기억 UI는 기억 전체 목록을 App 상태로 보관하고 전역·캐릭터 범위를 화면에서
필터링합니다. 활성 상태와 중요도 변경은 PATCH 응답으로 같은 ID 항목만 교체합니다.
